...
async fn send_requests(arguments: &CliArgs) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    while Instant::now() < generate_target_finish {
        ...
        // check if the processing thread is reporting any errors
    ...
    // wait for the response processing to complete or an error
    if let Some(error) = rx_control.recv().await {
        println!("Error processing results.");
        return Err(error.into());
    }
    ...
    println!(
        ...
        request_id as f32 / response_duration.as_secs_f32()
    );
