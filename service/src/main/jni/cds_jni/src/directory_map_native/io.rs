...
impl<'a> Read for JNIRead<'a> {
    fn read(&mut self, buf: &mut [u8]) -> Result<usize, std::io::Error> {
        self.buffer.ensure_buffer_capacity(buf.len())?;
        let result = self
            .env
            .call_method(
                self.input_stream,
...
impl<'a> Write for JNIWrite<'a> {
    fn write(&mut self, buf: &[u8]) -> Result<usize, std::io::Error> {
        self.buffer.ensure_buffer_capacity(buf.len())?;
        self.env
            .set_byte_array_region(*self.buffer.buffer.as_ref().unwrap().as_obj(), 0, transmute_byte_slice(buf))
            .map_err(|x| IoError::from(x))?;
        self.env
            .call_method(
                self.output_stream,
