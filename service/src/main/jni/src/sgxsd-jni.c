...
jbyteArray sgxsd_jni_get_byte_array_field(JNIEnv *env, jclass class, jobject object, const char *field_name) {
    jfieldID field_id = (*env)->GetFieldID(env, class, field_name, "[B");
    if (field_id == NULL) {
        return NULL;
    }
    return (*env)->GetObjectField(env, object, field_id);
}

jint sgxsd_jni_get_int_field(JNIEnv *env, jclass class, jobject object, const char *field_name) {
    jfieldID field_id = (*env)->GetFieldID(env, class, field_name, "I");
    if (field_id == NULL) {
        return 0;
    }
...
sgx_status_t sgxsd_ocall_reply(const sgxsd_msg_header_t *p_reply_header,
    ...
    jbyteArray j_reply_data = NULL;
    jbyteArray j_reply_iv   = NULL;
    jbyteArray j_reply_mac  = NULL;

    if (reply_data != NULL) {
        j_reply_data = sgxsd_jni_to_byte_array(env, reply_data, (jsize) reply_data_size);
    }

    if (p_reply_header != NULL) {
        j_reply_iv   = sgxsd_jni_to_byte_array(env, p_reply_header->iv.data,  sizeof(p_reply_header->iv.data));
        j_reply_mac  = sgxsd_jni_to_byte_array(env, p_reply_header->mac.data, sizeof(p_reply_header->mac.data));
    }
...
JNIEXPORT jlong JNICALL SGXSD_JNI_CLASS_METHOD(nativeServerStart)
    (JNIEnv *env, jclass class, jlong j_enclave_id, jlong j_server_state, jint j_max_query_phones) {
    sgx_enclave_id_t enclave_id = j_enclave_id;
    sgxsd_server_state_handle_t server_state = j_server_state;

    cds_start_args_t cds_start_args = {
        .max_query_phones     = j_max_query_phones,
        .max_ratelimit_states = 0,
    };
    sgx_status_t server_start_res;
    sgx_status_t server_start_ecall_res =
      sgxsd_enclave_server_start(enclave_id, &server_start_res, &cds_start_args, server_state);
...
void sgxsd_jni_server_call(JNIEnv *env, sgx_enclave_id_t enclave_id, sgxsd_server_state_handle_t server_state,
                           ...
                           uint8_t *msg_data, size_t msg_size,
...
JNIEXPORT void JNICALL SGXSD_JNI_CLASS_METHOD(nativeServerCall)
     (JNIEnv *env, jclass class, jlong j_enclave_id, jlong j_server_state, jobject j_call_args, jobject j_callback_obj) {
    jclass j_call_args_class = (*env)->FindClass(env, SGXSD_JNI_PACKAGE_STR "SgxEnclave$NativeServerCallArgs");
    if (j_call_args_class == NULL) {
    ...
    jint       j_query_phone_count  = sgxsd_jni_get_int_field(       env, j_call_args_class, j_call_args, "query_phone_count");
    jbyteArray j_query_data         = sgxsd_jni_get_byte_array_field(env, j_call_args_class, j_call_args, "query_data");
    jbyteArray j_query_iv           = sgxsd_jni_get_byte_array_field(env, j_call_args_class, j_call_args, "query_iv");
    jbyteArray j_query_mac          = sgxsd_jni_get_byte_array_field(env, j_call_args_class, j_call_args, "query_mac");
    jbyteArray j_query_commitment   = sgxsd_jni_get_byte_array_field(env, j_call_args_class, j_call_args, "query_commitment");
    jbyteArray j_msg_data           = sgxsd_jni_get_byte_array_field(env, j_call_args_class, j_call_args, "msg_data");
    jbyteArray j_msg_iv             = sgxsd_jni_get_byte_array_field(env, j_call_args_class, j_call_args, "msg_iv");
    jbyteArray j_msg_mac            = sgxsd_jni_get_byte_array_field(env, j_call_args_class, j_call_args, "msg_mac");
    jbyteArray j_pending_request_id = sgxsd_jni_get_byte_array_field(env, j_call_args_class, j_call_args, "pending_request_id");

    if (j_query_data == NULL ||
        j_query_iv == NULL ||
        j_query_mac == NULL ||
        j_query_commitment == NULL ||
        j_msg_data == NULL ||
        j_msg_iv == NULL ||
        j_msg_mac == NULL ||
    ...
    cds_call_args_t call_args = {
        .query_phone_count = j_query_phone_count,
        .query = (cds_encrypted_msg_t) {
            .iv   = (sgxsd_aes_gcm_iv_t)  { .data = { 0 }},
            .mac  = (sgxsd_aes_gcm_mac_t) { .data = { 0 }},
            .data = NULL,
            .size = 0,
        },
        .query_commitment = { 0 },
        .ratelimit_state_uuid = { .data64 = { 0, 0 }},
        .ratelimit_state_data = NULL,
        .ratelimit_state_size = 0,
    };

    (*env)->GetByteArrayRegion(env, j_query_iv, 0, sizeof(call_args.query.iv.data), (void *) &call_args.query.iv.data);
    (*env)->DeleteLocalRef(env, j_query_iv);

    (*env)->GetByteArrayRegion(env, j_query_mac, 0, sizeof(call_args.query.mac.data), (void *) &call_args.query.mac.data);
    (*env)->DeleteLocalRef(env, j_query_mac);

    (*env)->GetByteArrayRegion(env, j_query_commitment, 0, sizeof(call_args.query_commitment), (void *) &call_args.query_commitment);
    (*env)->DeleteLocalRef(env, j_query_commitment);
    ...
    if ((*env)->ExceptionCheck(env) != JNI_TRUE) {
        size_t msg_size;
        msg_data = sgxsd_jni_copy_byte_array(env, j_msg_data, &msg_size);
        if (msg_data != NULL) {
            size_t query_size    = 0;
            call_args.query.data = query_data = sgxsd_jni_copy_byte_array(env, j_query_data, &query_size);
            call_args.query.size = (uint32_t) query_size;
            if (query_data != NULL) {
                sgxsd_jni_server_call(env, enclave_id, server_state, &call_args, msg_data, msg_size, msg_iv, msg_mac, pending_request_id, j_callback_obj, j_callback_method_id);
            } else {
                sgxsd_jni_throw_sgxsd_exception(env, "bad_query_data", 0);
            }
        } else {
...
JNIEXPORT void JNICALL SGXSD_JNI_CLASS_METHOD(nativeServerStop)
    ...
    if (j_in_phone_count <= j_in_phones_capacity / ((jlong) sizeof(*(((cds_stop_args_t *) 0)->in_phones))) &&
        j_in_phone_count <= j_in_uuids_capacity  / ((jlong) sizeof(*(((cds_stop_args_t *) 0)->in_uuids)))) {
        cds_stop_args_t cds_stop_args = {
            .in_phones = in_phones,
            .in_uuids = in_uuids,
        ...
        sgx_status_t stop_ecall_res =
            sgxsd_enclave_server_stop(enclave_id, &stop_res, &cds_stop_args, server_state);
