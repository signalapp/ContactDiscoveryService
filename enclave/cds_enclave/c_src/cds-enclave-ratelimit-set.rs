...
#[no_mangle]
pub extern "C" fn cds_ratelimit_set_add(p_slots_data: *mut u8, slots_data_size: usize, p_query_phones: *const u64, query_phones_count: usize) {
    unsafe {
...
where T: AsRef<[u8]> + AsMut<[u8]>,
{
    fn slots_mut<'a>(&'a mut self) -> impl Iterator<Item = RatelimitSetSlot<&'a mut [u8]>> + 'a {
        self.data
            .as_mut()
            .chunks_exact_mut(size_of::<__m256i>())
            .map(RatelimitSetSlot)
    }
        ...
        for query_phone in query_phones {
            ...
            for mut slot in self.slots_mut() {
                slot.set(
                    _mm256_xor_si256(
                        slot.get(),
                        _mm256_set_epi64x(0, 0, 0, u64::MAX as i64)
                    )
                );
                ...
                insert_query_phone_block = _mm256_and_si256(insert_query_phone_block, _mm256_set1_epi8((query_phone_inserted != 0) as i8 - 1));
            }
        }
    }
}
